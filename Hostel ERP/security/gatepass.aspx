<%@ Page Title="" Language="C#" MasterPageFile="~/security/security.Master" AutoEventWireup="true" CodeBehind="gatepass.aspx.cs" Inherits="Hostel_ERP.security.WebForm3" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title>Gatepass Log</title>
</asp:Content>
   ...
   <main class="mt-5 pt-3">
        <div class="container-fluid">
            <div class="row justify-content-center card-header mb-3">
                <div class="col-md-3">
                    <h4 class="fw-bold">Gatepass Log</h4>
                </div>
            </div>

            <div class="row">
                <asp:SqlDataSource ID="SqlDataSourceGatePass" runat="server" ConnectionString="<%$ ConnectionStrings:Hostel_StudentConnectionString %>" SelectCommand="SELECT * FROM [student_gatepass] WHERE ([status] = @status)" DeleteCommand="DELETE FROM [student_gatepass] WHERE [id] = @id" InsertCommand="INSERT INTO [student_gatepass] ([enrollment], [name], [room], [date], [time ], [reason], [status], [otp]) VALUES (@enrollment, @name, @room, @date, @time_, @reason, @status, @otp)" UpdateCommand="UPDATE [student_gatepass] SET [enrollment] = @enrollment, [name] = @name, [room] = @room, [date] = @date, [time ] = @time_, [reason] = @reason, [status] = @status, [otp] = @otp WHERE [id] = @id">
                    <DeleteParameters>
                ...
                <asp:GridView ID="GridViewGatePass" CssClass="table table-primary table-striped table-hover" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" CellPadding="4" DataSourceID="SqlDataSourceGatePass" ForeColor="Black" GridLines="Horizontal" AutoGenerateColumns="False" DataKeyNames="id" ShowHeaderWhenEmpty="True" EmptyDataText="No Record Found" OnRowDataBound="GridViewGatePass_RowDataBound">
                    <Columns>
                        ...
                        <ControlStyle CssClass="btn btn-dark text-center" />
                        </asp:CommandField>
                    </Columns>
